$(document).ready(function () {
    $(document).on("click", ".sortDirectElementAssociation", function (e) {
        sortAssociationForTwoTypesMap($(this), "selectToDirectMap", renderDirectoryViewForDirectElementAssociationView);
        e.stopPropagation();
    });

    $(document).on("click", ".singleview-direct-element-association-view", function (e) {
        renderDirectElementAssociationViewData($(this), false);
        e.stopPropagation();
    });

    $(document).on("click", ".biview-direct-element-association-view", function (e) {
        renderDirectElementAssociationViewData($(this), true);
        e.stopPropagation();
    });

    $(document).on("click", ".selectToDirectMap", function (e) {
        let element = backing.dictionary_of_unique_id_to_attr_object[$(this).attr("uniqueId")];
        getSelectionIconOfMap(element, $(this));
        e.stopPropagation();
    });

    $(document).on("mouseleave", ".selectToDirectMap", function (e) {
        let desktop = $(this).closest(".innerDesktop").attr("id");
        $("#" + desktop).find(".checkbox-map .tooltip").addClass("dis-none");
        e.stopPropagation();
    });

    $(document).on("click", ".directElementMap", function () {
        $("#saveData").show();
        let data = {};
        ...
        if (view === "singleview") {
            ...
            data.shieldId = $("#" + viewId + "_elementSelectorAssociationSelected").attr("elementId");
        ...
        $("#" + viewId + " .selectToDirectMap").each(function () {
        ...
        service.saveDirectElementsAssociations(data, function (res, err) {
            if (res) {
    ...
    $(document).on("click", ".shieldExpAssociationSearchObjectItem", function () {
    ...
    $(document).on("click", ".shieldExpAssociationCustomSearchObjectItem", function () {
        let searchDropDownSelected, dropDownClass;
        let desktop = $(this).closest(".innerDesktop");
        let selectedElement = $(this);
        if (desktop.attr("view") === "singleview") {
    ...
    $(document).on("click", ".biShieldAssociationSearchObjectItem", function () {
        let selectedElement = $(this);
    ...
    $(document).on("click", ".singleviewDirectElementAssociationView", function (e) {
        renderSelectorDirectElementAssociationView($(this), false, "singleSearchElementTypeShieldExpAssociationDropDown");
        e.stopPropagation();
    });

    $(document).on("click", ".biviewDirectElementAssociationView", function (e) {
        renderSelectorDirectElementAssociationView($(this), true, "biSearchElementTypeShieldExpAssociationDropDown");
        e.stopPropagation();
    });
});

function renderSelectorDirectElementAssociationView(selector, isBiview, searchClass) {
    $("#saveData").show();
    let data = {};
    ...
    service.getDirectShieldElementAssociationsForShieldElement(data, function (res, err) {
        if (res) {
            ...
            renderDirectoryViewForDirectElementAssociationView(res, associationViewDivId, viewClass);
    ...
    let viewId = $("#" + associationViewDivId + "_elementSelectorAssociationSelected");
    viewId.html(selectedElement.find("span").html());
    viewId.attr("elementId", selectedElement.attr("elementId"));
    $("#" + associationViewDivId + "_elementSelectorAssociationSelected_dropDown_content").addClass("dis-none");
}

function renderDirectElementAssociationViewData(selector, isBiview) {
    $("#saveData").show();
    let activeWorkspaceKeyname, activeDesktopKeyname, activeDesktop, getDropDownService, viewClass;
    ...
    let associationViewDivId = backing.view_type.direct_shield_association_view.name + "_" + src_hotlinkId;
    ...
    let haveSpaceToOpenView = checkIfViewCanOpen(backing.view_type.direct_shield_association_view.key);
    ...
    if (protectedObjectType === "shield")
        getDropDownService = service.getShieldOfShieldType;
    else if (protectedObjectType === "standard")
    ...
    getDropDownService(data, function (shieldRes, err) {
        if (shieldRes) {
            if (shieldRes.children.length > 0) {
                ...
                service.getDirectShieldElementAssociationsForShieldElement(data, function (orgRes, err) {
                    if (orgRes) {
                        if (orgRes.children !== undefined) {
                            ...
                            generateUniqueIdAndParentLink(res, "directElementAssociationView", null);
                            renderDirectElementAssociationView(res, associationViewDivId, data, element, isBiview, selectedIconType);
                            renderDirectElementDropDownForAssociationView(shieldRes.children, viewClass, associationViewDivId, selectedShield);
                            renderDirectoryViewForDirectElementAssociationView(res, associationViewDivId, viewClass);
                            if (active_desktop_directory_view_id === src_id)
                                createScenarioViewOpenedFromAnchorSingleCase(associationViewDivId, active_desktop_directory_view_id, src_hotlinkId, backing.view_type.direct_shield_association_view.key, "Create Links: " + element[ATTR.name], uniqueId, isBiview, res);
                            else
                                createScenarioViewOpenedFromView(associationViewDivId, src_id, src_hotlinkId, backing.view_type.direct_shield_association_view.key, "Create Links: " + element[ATTR.name], uniqueId, isBiview, res);
                ...
                $("#saveData").hide();
                alert("There are only one shield,so this view is not applicable");
            }
        }
        else if (err) {
...
function renderDirectElementAssociationView(res, viewName, data, element, isBiview, associationObjectType) {
    ...
    let str = "";
    str += "<div objectType = \"" + associationObjectType + "\" id=\"" + viewName + "\" class=\"innerDesktop associationViewDesktop\" view=\"" + viewClass + "\" uniqueid=\"" + element.uniqueId + "\" expressionId = \"" + data.expressionId + "\">" +
    ...
    str += "<span class=\"fright sortIcon sortDirectElementAssociation flaticon-sort unSortAss\" title=\"Sort\"></span></div>";
    ...
    str += "<div class=\"desktop-selector-dropdown  fleft\">" +
        "<span id=\"" + viewName + "_elementSelectorAssociationSelected\" class=\"selector shieldSelector shieldSelectorAssociationSelected\">SELECT</span>" +
        "</div>" +
        "<span class=\"flaticon-search openSearch fright hideAndShowSearch\" title=\"Show Search Bar\"></span>" +
        "<span class=\"headerActionButton directElementMap\">SAVE</span>" +
        "</div>" +
...
function renderDirectoryViewForDirectElementAssociationView(data, viewId, view) {
    let isBiview = isBiViewOrNot(view);
    let associationViewId = viewId + "_association_tree_container";
    renderDirectoryView(data, associationViewId, view, "directElementAssociationView", isBiview);
}

function renderDirectElementDropDownForAssociationView(res, view, viewId, selectedShield) {
    ...
    for (let i = 0; i < res.length; i++) {
        str += "<li class = \"" + view + "DirectElementAssociationView grpSelector\" elementId=\"" + res[i][ATTR.elementId] + "\"><span>";
    ...
    str += "</ul>";
    selectorId = viewId + "_elementSelectorAssociationSelected_dropDown_content";
    $("#" + selectorId).html(str);
    if (defaultShield) {
        let selectedId = $("#" + viewId + "_elementSelectorAssociationSelected");
        let title = "";
        ...
        if (view === "singleview") {
            ...
            selectedId.attr("objectType", defaultShield.objectType);
