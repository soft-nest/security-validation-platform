$(document).ready(function () {
    ...
    $(document).on("click", ".sortDirectAssetElementAssociation", function (e) {
        sortAssociationForTwoTypesMap($(this), "selectDirectAssetMap", renderDirectoryViewForDirectAssetElementAssociationView);
        e.stopPropagation();
    });

    $(document).on("click", ".singleview-direct-asset-element-association-view", function (e) {
        renderdirectAssetElementAssociationViewData($(this), false);
        e.stopPropagation();
    });

    $(document).on("click", ".biview-direct-asset-element-association-view", function (e) {
        renderdirectAssetElementAssociationViewData($(this), true);
        e.stopPropagation();
    });

    $(document).on("click", ".selectDirectAssetMap", function (e) {
        let element = backing.dictionary_of_unique_id_to_attr_object[$(this).attr("uniqueId")];
        getSelectionIconOfMap(element, $(this));
        e.stopPropagation();
    });

    $(document).on("mouseleave", ".selectDirectAssetMap", function (e) {
        let desktop = $(this).closest(".innerDesktop").attr("id");
        $("#" + desktop).find(".checkbox-map .tooltip").addClass("dis-none");
        e.stopPropagation();
    });

    $(document).on("click", ".directAssetElementMap", function (e) {
        $("#saveData").show();
        let data = {};
        ...
        if (view === "singleview") {
            ...
            data.shieldId = $("#" + viewId + "_assetElementSelectorAssociationSelected").attr("elementId");
        ...
        $("#" + viewId + " .selectDirectAssetMap").each(function () {
        ...
        let saveFunction = (desktop.attr("isBusiness") === "true") ? service.saveDirectShieldElementAssociationsForBusinessAsset : service.saveDirectShieldElementAssociationsForAsset;
    ...
    $(document).on("click", ".singleviewDirectAssetElementAssociationView", function (e) {
        renderDirectAssetElementAssociationViewSelectorClick($(this), false, "singleSearchElementTypeShieldExpAssociationDropDown");
        e.stopPropagation();
    });

    $(document).on("click", ".biviewDirectAssetElementAssociationView", function (e) {
        renderDirectAssetElementAssociationViewSelectorClick($(this), true, "biSearchElementTypeShieldExpAssociationDropDown");
        e.stopPropagation();
    });
});

function renderDirectAssetElementAssociationViewSelectorClick(selector, isBiview, searchClass) {
    $("#saveData").show();
    let data = {};
    ...
    service.getDirectShieldElementAssociationsForAsset(data, function (res, err) {
        if (res) {
            ...
            renderDirectoryViewForDirectAssetElementAssociationView(res, associationViewDivId, viewClass, true);
    ...
    let selectedId = $("#" + associationViewDivId + "_assetElementSelectorAssociationSelected");
    selectedId.html(selectedElement.find("span").html());
    selectedId.attr("elementId", selectedElement.attr("elementId"));
    $("#" + associationViewDivId + "_assetElementSelectorAssociationSelected_dropDown_content").addClass("dis-none");
}

function renderdirectAssetElementAssociationViewData(selector, isBiview) {
    $("#saveData").show();
    ...
    let associationViewDivId = backing.view_type.direct_asset_element_association_view.name + "_" + src_hotlinkId;
    ...
    let getDropDownService;
    //check for space in screen
    let haveSpaceToOpenView = checkIfViewCanOpen(backing.view_type.direct_asset_element_association_view.key);
    ...
    let renderFunction = (isBusiness) ? service.getDirectShieldElementAssociationsForBusinessAsset : service.getDirectShieldElementAssociationsForAsset;
            ...
            renderFunction(data, function (orgRes, err) {
                if (orgRes) {
                    if (orgRes.children !== undefined) {
                        ...
                        generateUniqueIdAndParentLink(res, "directAssetElementAssociationView", null);
                        renderDirectAssetElementAssociationView(res, associationViewDivId, data, element, isBiview, selectedIconType, isBusiness);
                        renderDirectAssetElementDropDownForAssociationView(shieldRes.children, viewClass, associationViewDivId, selectedShield);
                        renderDirectoryViewForDirectAssetElementAssociationView(res, associationViewDivId, viewClass);
                        if (active_desktop_directory_view_id === src_id)
                            createScenarioViewOpenedFromAnchorSingleCase(associationViewDivId, active_desktop_directory_view_id, src_hotlinkId, backing.view_type.direct_asset_element_association_view.key, "Create Links: " + element[ATTR.name], uniqueId, isBiview, res);
                        else
                            createScenarioViewOpenedFromView(associationViewDivId, src_id, src_hotlinkId, backing.view_type.direct_asset_element_association_view.key, "Create Links: " + element[ATTR.name], uniqueId, isBiview, res);
                        let objectTypesAndElementTypes = objectTypeInScope.processObjectTypeDropDownContent(res);
                        if (objectTypesAndElementTypes) {
                            renderObjectTypesAndElementTypesDropDownShieldExp(objectTypesAndElementTypes, associationViewDivId, searchClass, "shieldExpAssociationSearchObjectItem");
                        }
...
function renderDirectAssetElementAssociationView(res, viewName, data, element, isBiview, associationObjectType, isBusiness) {
    ...
    str += "<span class=\"fright sortIcon sortDirectAssetElementAssociation flaticon-sort unSortAss\" title=\"Sort\"></span>";
    str += "</div>" +
        ...
        "</div>" +
        "<span class=\"typeIcon\" title=\"" + element.name.toUpperCase() + "\">" + objectTypeIcon + "" + element.name.toUpperCase() + "</span></div>";
    str += "<div class=\"desktop-selector-container " + viewClass + "\">" +
    ...
    str += "<div class=\"desktop-selector-dropdown fleft\">" +
        "<span id=\"" + viewName + "_assetElementSelectorAssociationSelected\" class=\"selector shieldSelector shieldSelectorAssociationSelected\">SELECT</span>" +
        "</div>" +
        "<span class=\"flaticon-search openSearch fright hideAndShowSearch\" title=\"Show Search Bar\"></span>" +
        "<span class=\"headerActionButton directAssetElementMap\">SAVE</span>" +
        "</div>" +
...
function renderDirectoryViewForDirectAssetElementAssociationView(data, viewId, view) {
    let isBiview = isBiViewOrNot(view);
    let associationViewId = viewId + "_association_tree_container";
    renderDirectoryView(data, associationViewId, view, "directAssetElementAssociationView", isBiview);
}

function renderDirectAssetElementDropDownForAssociationView(res, view, viewId, selectedShield) {
    ...
    for (let i = 0; i < res.length; i++) {
        str += "<li class = \"" + view + "DirectAssetElementAssociationView grpSelector\" elementId=\"" + res[i][ATTR.elementId] + "\"><span>";
    ...
    str += "</ul>";
    selectorId = viewId + "_assetElementSelectorAssociationSelected_dropDown_content";
    $("#" + selectorId).html(str);
    if (defaultShield) {
        let selectedId = $("#" + viewId + "_assetElementSelectorAssociationSelected");
        let title = "";
