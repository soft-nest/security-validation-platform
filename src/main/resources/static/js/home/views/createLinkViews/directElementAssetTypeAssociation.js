$(document).ready(function () {
    ...
    $(document).on("click", ".sortDirectAssetTypeAssociation", function (e) {
        sortAssociationForTwoTypesMap($(this), "selectDirectAssetTypeAssociationMap", renderDirectoryViewForDirectAssetTypeAssociationView);
        e.stopPropagation();
    });

    $(document).on("click", ".singleview-direct-asset-type-association-view", function (e) {
        renderdirectShieldAssetTypeAssociationViewData($(this), false);
        e.stopPropagation();
    });

    $(document).on("click", ".biview-direct-asset-type-association-view", function (e) {
        renderdirectShieldAssetTypeAssociationViewData($(this), true);
        e.stopPropagation();
    });

    $(document).on("click", ".selectDirectAssetTypeAssociationMap", function (e) {
        let element = backing.dictionary_of_unique_id_to_attr_object[$(this).attr("uniqueId")];
        getSelectionIconOfMap(element, $(this));
        e.stopPropagation();
    });

    $(document).on("mouseleave", ".selectDirectAssetTypeAssociationMap", function (e) {
        let desktop = $(this).closest(".innerDesktop").attr("id");
        $("#" + desktop).find(".checkbox-map .tooltip").addClass("dis-none");
        e.stopPropagation();
    });

    $(document).on("click", ".directAssetTypeMap", function () {
        $("#saveData").show();
        ...
        let data = {};
        let associatedAssetTypes = [];
        $("#" + viewId + " .selectDirectAssetTypeAssociationMap").each(function () {
            if ($(this).hasClass("flaticon-arrow")) {
                associatedAssetTypes.push($(this).attr("id"));
            }
        });
        data.associatedAssetTypes = associatedAssetTypes;
        data.shieldElementId = element[ATTR.elementId];
        let saveFunction = (desktop.attr("isBusiness") === "true") ? service.saveDirectBusinessAssetTypeToShieldElementAssociations : service.saveDirectAssetTypeToShieldElementAssociations;
...
function renderdirectShieldAssetTypeAssociationViewData(selector, isBiview) {
    $("#saveData").show();
    ...
    let associationViewDivId = backing.view_type.direct_asset_type_association_view.name + "_" + src_hotlinkId;
    let uniqueId = selector.attr("uniqueid");
    let element = backing.dictionary_of_unique_id_to_attr_object[uniqueId];
    let data = {};
    data.elementId = element[ATTR.elementId];
    let isBusiness = (selector.attr("objectType") === "business_asset_type") ? true : false;
    //check for space in screen
    let haveSpaceToOpenView = checkIfViewCanOpen(backing.view_type.direct_asset_type_association_view.key);
    ...
    let renderFunction = (isBusiness) ? service.getDirectBusinessAssetTypeAssociationsForElement : service.getDirectAssetTypeAssociationsForElement;
    renderFunction(data, function (orgRes, err) {
        if (orgRes) {
            if (orgRes.children !== undefined) {
                ...
                renderDirectAssetTypeAssociationView(res, associationViewDivId, element, isBiview, isBusiness);
                if (res[ATTR.children])
                    renderDirectoryViewForDirectAssetTypeAssociationView(res, associationViewDivId, viewClass);
                if (active_desktop_directory_view_id === src_id)
                    createScenarioViewOpenedFromAnchorSingleCase(associationViewDivId, active_desktop_directory_view_id, src_hotlinkId, backing.view_type.direct_asset_type_association_view.key, "Create Links: " + element[ATTR.name], uniqueId, isBiview, res);
                else
                    createScenarioViewOpenedFromView(associationViewDivId, src_id, src_hotlinkId, backing.view_type.direct_asset_type_association_view.key, "Create Links: " + element[ATTR.name], uniqueId, isBiview, res);
                ...
                alert("No Asset Types Create Links");
            }
            $("#saveData").hide();
        }
        else if (err) {
...
function renderDirectAssetTypeAssociationView(res, viewName, element, isBiview, isBusiness) {
    ...
    str += "<span class=\"fright sortIcon sortDirectAssetTypeAssociation flaticon-sort\" title=\"Sort\"></span>";
    str += "</div>" +
        ...
        "<span class=\"typeIcon\" title=\"" + headerName.toUpperCase() + "\">" + headerName.toUpperCase() + "</span></div>" +
        "<div class=\"desktop-selector-container " + viewClass + "\" style=\"color:#000;\">" +
    ...
    str += "<span class=\"flaticon-search openSearch fright hideAndShowSearch\" title=\"Show Search Bar\"></span>" +
        "<span class=\"headerActionButton directAssetTypeMap\" style=\"padding: 0;\">SAVE</span></div>" +
...
function renderDirectoryViewForDirectAssetTypeAssociationView(data, viewId, view) {
    let isBiview = isBiViewOrNot(view);
    let associationViewId = viewId + "_association_tree_container";
    renderDirectoryView(data, associationViewId, view, "directAssetTypeAssociationView", isBiview);
}
